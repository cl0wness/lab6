...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if(comLine.arguments().size()>6) // (2 ветка) берём из main
        {
            input = comLine.arguments().at(3); // берём из 2 ветки
            output = comLine.arguments().at(4); // берём из обеих
        }
        // Иначе - исключение 0
        else
            throw 0;
    ...
    catch (int error)
    {
        ...
        try {
            ...
            saveToFile(input, exception);  // (4 ветка) берем из main
        } catch (bool error) {             // берём из 4 ветки
            // Напечатать предупреждение, если нет доступа к выходному файлу
            if(error != 1)                 // берем из обеих
...
{
    int letters = 0;                  // (3 ветка) берем из main
    //..Считать слова одинаковыми
    int equal = true;                 // берем из 3 ветки
    ...
    while(equal && first.length()>0 && second.length()>0)
    {
        ...
        if(first[1] == second[0])    // берем из обеих
        {
            first.remove(0,1);
            second.remove(0,1);
            delLetters++;
